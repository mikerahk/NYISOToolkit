...
class NYISOStat:
    ...
    def table_average_day_load(year):
        ...
        df2 = NYISOStat.table_hourly_dataset(dataset="fuel_mix_5m", year=year)[
    ...
    def table_annual_energy(year="2019"):
        ...
        imports = NYISOData(dataset="interface_flows_5m", year="2019").df.tz_convert(
            "US/Eastern"
        )
        imports.drop(
        ...
        fuel_mix = fuel_mix.rename(columns={0: f"Historic ({year})"}).sort_values(
            f"Historic ({year})", ascending=False
        )
