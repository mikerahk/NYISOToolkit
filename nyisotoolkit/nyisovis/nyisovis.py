...
class NYISOVis:
    ...
    def dfs_clcpa_carbon_free(year="2019", f="D"):
        ...
        dfs["ef"]["percent_carbon_free"] = dfs["ef"][CARBONFREE_SOURCES].sum(
            axis="columns"
        )
    ...
    @staticmethod
    def fig_clcpa_carbon_free(
        year="2019", f="D", out_dir=pl.Path(c_dir, "visualizations")
    ):
        """
        ...
        for t, l, c, h in zip(data, averages, colors, h_distances):
            avg = (
                dfs["fuel_mix"][t].sum(axis="index").sum()
                / dfs["load"].sum(axis="index")
                * 100
            )
            avg_imp = (
                dfs["fuel_mix"][t + ["Imports"]].sum(axis="index").sum()
                / dfs["load"].sum(axis="index")
                * 100
            )
        ...
        ax.set(
            ...
            xlim=[dfs["df"].index[0], dfs["df"].index[-1]],
            ylim=[0, 100],
        )
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%b"))
        plt.setp(ax.get_xticklabels(), rotation=0, horizontalalignment="center")
    ...
    def fig_carbon_free_year(year="2019", out_dir=pl.Path(c_dir, "visualizations")):
        ...
        df = stats[f"Historic ({year}) [% of Load]"].drop(
            index=[
        ...
        # Averages and Goals
        # Carbon Free Line
        perc = stats[f"Historic ({year}) [% of Load]"].loc[
            "Total Carbon-Free Generation"
        ]
        ax.axhline(
